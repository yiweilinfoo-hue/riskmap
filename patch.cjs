...
code = code.replace("export function RiskList", cardDef + "\nexport function RiskList");

// Find the card rendering block and replace it
const startPattern = "<div\\s+key=\\{item\\.id\\}\\s+onClick=\\{\\(\\) => toggleExpand\\(item\\.id\\)\\}";
const startIndex = code.search(new RegExp(startPattern));
if (startIndex !== -1) {
    let endIndex = code.indexOf("</AnimatePresence>", startIndex);
    if (endIndex !== -1) {
        endIndex = code.indexOf("</div>", endIndex + 18);
        if (endIndex !== -1) {
            const before = code.substring(0, startIndex);
            const after = code.substring(endIndex + 6);
            code = before + "<RiskCard key={item.id} item={item} isExpanded={isExpanded} toggleExpand={toggleExpand} triggerToast={triggerToast} />" + after;
        }
    }
}
